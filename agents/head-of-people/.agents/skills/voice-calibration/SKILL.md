...
description: Use when the user says "calibrate my HR voice" / "sample my past offers" / "learn how I write for HR comms" — pulls recent outbound HR messages via a Composio-connected inbox (or pasted samples), extracts a tone fingerprint, and appends to the voice-notes section of `people-context.md`.
---

# Voice Calibration

Offers, rejections, team announcements, PIPs, stay conversations — the
other HR agents all draft against your voice. This skill samples how
you actually write HR comms and writes a tone fingerprint into
`people-context.md` that every downstream draft references.
...
1. **Read people-context doc** (own file): `people-context.md`. Read
...
3. **If connected, fetch.** Execute the discovered inbox tool's
   ...
   can't distinguish, ask the user for a label / folder name or a
...
8. **Append to `outputs.json`.** Read-merge-write atomically — this
   ...
   ```json
   {
     ...
     "summary": "<2-3 sentences — N samples mined, top 3 fingerprint notes, what changed in people-context.md>",
   ...
   artifact — the voice notes live inside `people-context.md`.)
...
- Updates the voice-notes section of `people-context.md` (live doc).
