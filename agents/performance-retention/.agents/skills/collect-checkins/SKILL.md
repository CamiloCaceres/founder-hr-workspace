...
description: Use when the user says "collect this week's check-ins" / "1:1 status across the team" / "who's been quiet" — pulls the roster from the connected HRIS, sends the check-in prompt to each member via the connected Slack channel, collects responses, and summarizes who responded, who's quiet, themes, and escalation-flagged responses. Writes a dated markdown report under `checkins/` and appends to `outputs.json`.
---
...
- Implicit: called by Head of People's `weekly-people-review` to pull
  fresh check-in state before the Monday readout.
- Frequency: typically matches `config/checkin-rhythm.json` cadence
...
1. **Read people-context doc:**
   ...
   the user: "I need the people-context doc first — please spend 5
   minutes with the Head of People (`define-people-context`)." Stop.
2. **Read config:** `config/roster.json` and
   `config/checkin-rhythm.json`. If either is missing, ask ONE
...
4. **Resolve the check-in prompt.** Use `defaultPrompt` from
   `config/checkin-rhythm.json` if set; otherwise read the
   check-in-prompt section of `people-context.md`; otherwise fall
...
5. **Send the prompt.** Run `composio search chat` to discover the
   ...
   exists, tell the user which category to link from the Integrations
   tab and stop.
6. **Collect responses** over the configured window. Read replies
   from the same channel / thread / DM. If responses are not
   retrievable programmatically, ask the user to paste the thread
   export.
7. **Summarize.** Per team member: responded / not-responded, and
   ...
   section of `people-context.md` (e.g. harassment, discrimination,
