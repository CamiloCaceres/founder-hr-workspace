...
description: Use when the user says "prep the review cycle" / "Q{N} reviews are starting" / "build the review templates" — produces the self-review template, manager-review template, calibration doc, and full timeline for the cycle, all scoped to the leveling framework in `people-context.md`. Writes to `review-cycles/{cycle-slug}.md` as `status: "draft"` until the founder approves the structure.
---
...
- Implicit: triggered by `weekly-people-review` when the upcoming
  cycle date from `people-context.md` is within the lead-time window.
...
2. **Read config:** `config/review-cycle.json` and
   `config/roster.json`. If the review-cycle rhythm is unset, use the
   one in `people-context.md`. If the roster source is
...
3. **Resolve the cycle slug.** Default to `YYYY-q{N}` (e.g.
   ...
   for annual. Ask the user if the default doesn't match their
   internal naming.
4. **Produce four artifacts** in one markdown file:
   ...
   - **Manager-review template** — the same attribute scaffolding,
     ...
     scale (if `people-context.md` defines one) and a
...
5. **Voice check.** Pull voice notes from `people-context.md` — the
...
hasn't written. If `people-context.md`'s leveling section is `TBD`,
