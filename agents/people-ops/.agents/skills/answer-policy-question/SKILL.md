...
description: Use when the user asks "does {employee} qualify for {benefit}" / "can {employee} expense {item}" / "what's our policy on {leave / remote / equipment}" — reads the policy canon AND escalation rules from `../head-of-people/people-context.md`, classifies the question into direct / ambiguous / escalation, and drafts the reply (or escalation note) accordingly. Never drafts policy answers for legal-sensitive escalation categories.
---
...
1. **Read people-context doc.** Read
   ...
   the user: "I need your people-context doc first — please spend 5
   minutes with the Head of People (`define-people-context`)." Stop.
2. **Read the escalation rules section specifically** from
   `people-context.md`. These define which categories route to a
...
3. **Classify the incoming question into exactly one of three
   ...
   - **Direct answer** — the question is covered by the policy canon
     in `people-context.md` (leave · benefits · expenses · remote ·
   ...
   - **Escalation required** — the question matches one of the
     ...
     else defined in the `people-context.md` escalation section).
...
4. **For direct answers — read voice + draft the reply.** Read
   ...
   `people-context.md`. Draft the reply in that voice, citing the
   specific policy section (e.g. "Per our PTO policy in
   people-context.md § Policy canon — 15 days accrued after 90-day
