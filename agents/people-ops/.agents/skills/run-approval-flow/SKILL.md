...
description: Use when the user says "review this {PTO / comp / promotion / expense} request" / "approve this ask" / "should we {X}" — reads the rubric from `../head-of-people/people-context.md`, evaluates the request, classifies approved / escalate / denied with reasoning, and produces an escalation note for out-of-rubric asks.
---
...
1. **Read people-context doc.** Read
   `../head-of-people/people-context.md` for the relevant rubric
   ...
   user: "I need your people-context doc first — please spend 5
   minutes with the Head of People (`define-people-context`)."
   Stop.
2. **Read config.** `config/hris.json` for any HRIS data needed to
