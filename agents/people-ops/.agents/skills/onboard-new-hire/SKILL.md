...
description: Use when the user says "draft the onboarding plan for {new hire}" / "first 90 days for {new hire}" / "{new hire} starts Monday — get them ready" — reads people-context for leveling + voice, then produces a Day 0 / Week 1 / 30-60-90 plan plus welcome Slack and welcome email drafts.
---
...
- Implicit: routed from Recruiter after `draft-offer` → offer
  accepted, or from Head of People after a new hire is confirmed.
- Frequency: once per new hire.

## Steps

1. **Read people-context doc.** Read
   `../head-of-people/people-context.md` for values, leveling
...
2. **Read config.** `config/hris.json` for the HRIS (read-only —
   the agent never modifies HRIS records) and `config/helpdesk.json`
...
5. **Draft the welcome Slack message + welcome email.** Read voice
   notes from `people-context.md` (and `config/voice.md` if
...
in `people-context.md`. If the level isn't defined, mark TBD and
