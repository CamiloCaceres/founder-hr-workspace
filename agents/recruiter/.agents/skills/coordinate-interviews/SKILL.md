...
description: Use when the user says "schedule {candidate}'s loop" / "coordinate the panel for {candidate}" / "set up interviews" — proposes a schedule via a Composio calendar tool (check free/busy, never send), runs `prep-interviewer` per panelist, and appends schedule + briefs to the loop file. Founder approves and sends.
---
...
2. **Read the candidate record** at
   `candidates/{candidate-slug}.md`. If missing, tell the user to
...
4. **Discover the calendar tool via Composio.** Run `composio search
   ...
   If no calendar is connected, tell the user which category to link
