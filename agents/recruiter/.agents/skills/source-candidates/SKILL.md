...
description: Use when the user says "find candidates for {role}" / "source engineers from GitHub" / "build a sourcing list for {role}" — given a role + signal source (GitHub · LinkedIn · community posts · conference lists · OSS repos), pulls candidates matching the role's rubric, scores each against the must-haves, and writes a ranked sourcing list.
---
...
- Implicit: kicked off by the founder at the start of a hiring push,
  or by the Head of People during a req-planning session.
...
3. **Read config**: `config/ats.json` (for duplicate checks later) and
   `config/reqs.json` (confirm the req is in the open list).
4. **Confirm the signal source** the user named (GitHub repo / org,
...
5. **Discover tools via Composio.** Run `composio search web-scrape`
   ...
   is unconnected, tell the user which to link from Integrations and
   stop.
